...
HELM_REPO_NAME ?= ghcr.io/jacobweinstock/release-testing/charts
...
CRANE_FQP := $(TOOLS_DIR)/crane-$(CRANE_VER)
#######################################
...
$(CRANE_FQP):
	GOBIN=$(TOOLS_DIR) go install github.com/google/go-containerregistry/cmd/crane@$(CRANE_VER)
	@mv $(TOOLS_DIR)/crane $(CRANE_FQP)

.PHONY: tools
tools: $(GOIMPORTS_FQP) $(CONTROLLER_GEN_FQP) $(PROTOC_GEN_GO_GRPC_FQP) $(PROTOC_GEN_GO_FQP) $(BUF_FQP) $(UPX_FQP) $(GODEPGRAPH_FQP) $(CRANE_FQP) ## Install all tools
